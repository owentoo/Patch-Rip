...
export const action = async ({ request, params }: ActionFunctionArgs) =>
  withTenant(request, async ({ shop, staffMember }) => {
    ...
    if (!staffMember) {
      return json(
        { ok: false, error: "Staff identity not available — cannot attribute upload." },
        { status: 400 },
      );
    }
    ...
    for (const f of files) {
      ...
      if (f.type && !ACCEPTED_MIME_TYPES.has(f.type)) {
        // Be lenient — some browsers report empty mime for AI/EPS/PSD.
      }
    }
    ...
    const proof = await ensureProofForLineItem({
      ...
      lineItemId,
      lineItemTitle,
      lineItemVariantTitle: lineItemVariantTitle && lineItemVariantTitle.length > 0 ? lineItemVariantTitle : null,
